package org.lwjglb.terrains;

import org.joml.Math;
import org.joml.Vector2f;
import org.joml.Vector3f;
import org.lwjglb.models.RawModel;
import org.lwjglb.renderEngine.Loader;
...
public class Terrain {
    ...
    public float getHeightOfTerrain(float worldX, float worldZ) {
       ...
       float answer;
       if (xCoord <= (1-zCoord)) {
            answer = baryCentric(new Vector3f(0, heights[gridX][gridZ], 0),
                                 new Vector3f(1, heights[gridX+1][gridZ],0),
                                 new Vector3f(0, heights[gridX][gridZ +1], 1),
                                 new Vector2f(xCoord, zCoord));
       } else {
           answer = baryCentric(new Vector3f(1, heights[gridX+1][gridZ], 0),
    ...
    private RawModel generateTerrain(Loader loader, String heighMap){
        ...
        try {
            image = ImageIO.read(new File( new Config().getPath() + "textures/" + heighMap + ".png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
